...
\section{Main idea}
\noindent A simple model conveys the main idea of this work.
...
We want to determine the separation  $r(t)=x_A(t)-x_B(t)$ between
$A$ and $B$ at time $t$. Now if we know the initial configuration of the system
then we have complete information to determine a future configuration
and any observable thereof, e.g.\ $r(t)$.
But suppose we know the exact initial positions,
$q_A\equiv x_A(0)$ and $q_B\equiv x_B(0)$, of only two particles.
We don't know the exact initial coordinates of particle $C$,
but we do it is roughly at $q_*\pm\sigma_*$. (For concreteness,
we assume it is Gaussian distributed about $q_*$ with variance $\sigma_*^2$.)
Since we need the positions of all three particles, we do \emph{not} have
enough information to exactly determine $r(t)$. Instead, because we
have a distribution of $q_C$, we also have a {distribution} of $r(t)$.
%This is by marginalizing over all $q_C\sim\mathcal{N}(q_*,\sigma_*^2)$
%for given $q=q_A-q_B$.
%And since the uncertainty in particle $C$'s position
%grows with time, so also the uncertainty of $r(t)$ grows with time.
